...
1. **Análisis léxico**: el código se divide en *tokens* (palabras clave, identificadores, números, etc.).
...
edad = 30; direccion = 'Calle 123'; telefono = '555-1234';

// Asignación múltiple
let nombre2 = 'Ana', apellido2 = 'González', edad2 = 25;
```
...
| Flecha (*arrow*) | Forma abreviada de una función expresiva: `const f = (a, b) => a + b;`. |

```javascript
function sumar(a, b) { return a + b; }         // declarativa
const sumar2 = function(a, b) { return a + b; }; // expresiva
...
El **DOM** (*Document Object Model*) es la representación en memoria de la estructura de una página web, que permite acceder y modificar sus elementos HTML, CSS y JavaScript en tiempo real. Es una técnica fundamental para crear páginas dinámicas y reactivas.
...
```javascript
console.log("Inicio");
setTimeout(function() { console.log("Después de 2 segundos"); }, 2000);
console.log("Fin");
...
const promesa = new Promise(function(resolve, reject) {
  const exito = true;
  if (exito) { resolve("La operación salió bien"); }
  else { reject("La operación falló"); }
});
```
...
obtenerUsuario()
  .then(function(usuario) { console.log("Usuario:", usuario); })
  .catch(function(error) { console.log("Error:", error); });
```
...
obtenerUsuario()
  ...
  .then(function(notas) { console.log("Notas:", notas); })
  .catch(function(error) { console.log("Error:", error); });
```
...
function descargarArchivo(nombreArchivo) {
  return new Promise(function(resolve, reject) {
    setTimeout(function() {
      if (!nombreArchivo) { reject("Debe indicar un nombre de archivo"); }
      else { resolve("Archivo descargado: " + nombreArchivo); }
    }, 2000);
  });
}
descargarArchivo("")
...
async function obtenerDatos() {
  try {
    const respuesta = await axios.get('https://jsonplaceholder.typicode.com/todos/1');
...
Una **API** (*Application Programming Interface*) es un conjunto de reglas, protocolos y herramientas para que aplicaciones o servicios interactúen entre sí e intercambien información de manera programática.
...
**JSON** (*JavaScript Object Notation*) es un formato de intercambio de datos ligero, similar a un objeto JavaScript, compuesto por pares clave-valor. Es el formato más común para intercambiar datos entre cliente y servidor.
