...
> **Actualizado:** ya no hace falta escribir `import React from 'react'` en cada archivo para poder usar JSX. Desde el nuevo transformador de JSX ("*automatic runtime*"), que Vite usa por defecto, solo se importa lo que realmente se necesita, por ejemplo `import { useState } from 'react'`.
