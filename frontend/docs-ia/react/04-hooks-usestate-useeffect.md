...
> **Buena práctica:** cuando el nuevo valor depende del anterior, es más seguro usar la forma funcional del setter: `setCount(c => c + 1)` en lugar de `setCount(count + 1)`. Esto evita bugs si React agrupa varias actualizaciones seguidas (*batching*).
